...
//     has no import of src/backend/integrations/calle/* anywhere.
...
// src/, not even a relative one), NOT src/backend/persistence/supabase-client.ts /
// SupabaseRepository / src/shared/utilities/phone.ts / src/shared/utilities/avatars.ts:
...
//     writing this file.
//   - Even a leaf, class-free module like src/shared/utilities/phone.ts hits a genuine
...
//   src/backend/persistence/supabase-client.ts does (same env vars, same client
...
// ── Duplicated from src/shared/utilities/phone.ts (see that file for the authoritative
...
// ── Duplicated from src/shared/utilities/avatars.ts (see that file for the authoritative
...
// ── Enum values, mirrored from src/shared/validation/profile.ts ───────────────────
...
// src/shared/validation/profile.ts's allowed values ever change, this list must be
...
// src/backend/persistence/seed.ts: "daughter", "son", "trusted neighbour"), including
...
// ── Supabase client (mirrors src/backend/persistence/supabase-client.ts's pattern) ────
...
// (phoneEnvVarFor/resolveConfiguredPhone in src/backend/persistence/seed.ts, which
...
// ── Row shapes (subset of src/backend/persistence/row-mappers.ts's PersonRow/ContactRow) ──
interface PersonRow {
  id: string;
  first_name: string;
  phone: string;
