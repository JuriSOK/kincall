...
import type { EventStatus, OrchestrationDecision } from "@/backend/orchestration/state-machine/states";
...
export interface VulnerablePerson {
  ...
  // src/shared/presentation/format-date.ts still renders every date in one fixed
  // Europe/Paris display zone until Stage D wires a person's own timezone in.
  timezone: string;
  // A preset identifier from src/shared/utilities/avatars.ts's AVATAR_KEYS, or null (falls
  // back to an initials display — see src/frontend/components/avatars/avatar.tsx). Never an
...
export interface TrustedContact {
  ...
  // makes the cascade wait — see src/backend/orchestration/cascade/contact-order.ts for the
  ...
  // is tried below src/backend/orchestration/engine.ts's MAX_CONTACT_ATTEMPTS, never
