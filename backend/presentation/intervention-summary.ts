...
// "at around 6pm" / "at vers 18h00" — see backend/orchestration/engine.ts's own
...
function describeAction(result: FamilyStructuredResult): { action: string; known: boolean } {
  switch (result.intervention_type) {
    ...
    // (backend/integrations/calle/fake-adapter.ts's noAnswer) — it carries no commitment of its
